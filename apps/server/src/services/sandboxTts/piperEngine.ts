/**
 * VENDORED FROM `packages/bot-streamer/src/tts/piperEngine.ts` — kept
 * byte-equivalent (only the relative `Mood` import path differs) so
 * the sandbox lipsync diagnostic exercises the same algorithm
 * production runs in `infra/streamer/Dockerfile`. The server image
 * cannot import `@price-game/bot-streamer` directly without pulling
 * Playwright (~300MB browser binaries) into the production alpine
 * Dockerfile, so this copy is the lesser of two evils. If the
 * upstream module changes, sync this file alongside.
 *
 ...
 * so callers can pass partial overrides. Each knob is clamped inside
 * `buildPiperArgs` before reaching the subprocess.
 */
